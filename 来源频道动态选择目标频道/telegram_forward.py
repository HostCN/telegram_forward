...
SOURCE_TO_TARGET_MAPPING = config.get("source_to_target_mapping", {})
...
async def main():
    async with TelegramClient(StringSession(SESSION_STRING), API_ID, API_HASH) as client:
        ...
        @client.on(events.NewMessage(chats=list(SOURCE_TO_TARGET_MAPPING.keys())))
        async def handler(event):
            message = event.message
            source_channel_id = str(event.chat_id)  # 来源频道 ID

            # 根据来源频道动态选择目标频道
            target_channels = SOURCE_TO_TARGET_MAPPING.get(source_channel_id, [])
            if not target_channels:
                logging.warning(f"来源频道 {source_channel_id} 没有对应的目标频道映射")
                return
